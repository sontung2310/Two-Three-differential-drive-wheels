...
N = 40;
vL2 = [ones(20,1)*3;ones(20,1)*2];
vR2 = [ones(20,1)*2;ones(20,1)*3];
vL = zeros(N);
vR = zeros(N);
% PNoise_L = rand(N,1);
% PNoise_R = rand(N,1);
...
% ONoise_y= rand(N,1);
% ONoise_t= rand(N,1);
system_noise = [0.6 0.6];
measurement_noise = [5 5 0.08];
dt = 0.05;
r_wheel = 0.5;
b_wheel = 0.6;
P = zeros(3,3);
...
while(i<=N)
    ...
    vL=vL2(i-1)+ random('Normal', 0, system_noise(1));
    vR=vR2(i-1)+ random('Normal', 0, system_noise(2));
    dSL=dt*r_wheel*vL;
    dSR=dt*r_wheel*vR;
    dS=(dSL+dSR)/2;
    ...
    pos_mes_X(i)=pos_true_X(i)+ random('Normal',0,measurement_noise(1));
    pos_mes_Y(i)=pos_true_Y(i)+ random('Normal',0,measurement_noise(2));
    pos_mes_T(i)=pos_true_T(i)+ random('Normal',0,measurement_noise(3));
...
%4. P va Kalman gain K
    ...
    W=dt*r_wheel/2*[cos(pos_est_T(i-1)+dTheta/2)-(dS/b_wheel)*sin(pos_est_T(i-1)+dTheta/2) ... % Jacobian of input noise
                    ...
                    1/b_wheel -1/b_wheel];
%     sig = sqrt((0.5^2)/12);
%     Q = [sig*(vL^2) 0; 0 sig*(vR^2)];
%     R = [sig 0 0; 0 sig 0; 0 0 sig];
    Q = [1*vR2(i)^2          0;
         0          1*vL2(i)^2];
    R = [10		0		0;
			 0		10 		0;
			 0		0		0.01];
    prior_P =A*P*A'+W*Q*W'; %3x3
    K=P/(P+R); %3x3
